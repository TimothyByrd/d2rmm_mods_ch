...
function AddSocketsRecipe(cubemainTsv, inQuality, numSockets) {
    ['tors', 'helm', 'shld', 'weap'].forEach((itemType) => {
        ...
        cubemainTsv.rows.push(recipe);
    });
}

function AddJewelryRecipe(cubemainTsv, inType, inQuality, outType, outQuality) {
    const recipe = {
...
function processFolder(folder) {
    ...
    if (config.jewelry) {
        ...
        AddJewelryRecipe(cubemain, 'rin', 'mag', 'amu', 'mag');
        AddJewelryRecipe(cubemain, 'rin', 'rar', 'amu', 'rar');

        addRecipeEntry('2 amulets -> ring');
        AddJewelryRecipe(cubemain, 'amu', 'mag', 'rin', 'mag');
        AddJewelryRecipe(cubemain, 'amu', 'rar', 'rin', 'rar');

        addRecipeEntry('2 magic jewels -> rare jewel');
        AddJewelryRecipe(cubemain, 'jew', 'mag', 'jew', 'rar');

        addRecipeEntry('2 rare jewels -> magic jewel');
        AddJewelryRecipe(cubemain, 'jew', 'rar', 'jew', 'mag');
    ...
    if (config.addSockets) {
        ...
        AddSocketsRecipe(cubemain, 'uni', 1);
        AddSocketsRecipe(cubemain,' set', 1);
        AddSocketsRecipe(cubemain, 'rar', 2);
        AddSocketsRecipe(cubemain, 'mag', 4);
    }
    ...
    if (config.specific) {
        ...
        const prefixesFound = {};
        const magicprefixFilename = folder + '\\magicprefix.txt';
        ...
        const suffixesFound = {};
        const magicsuffixFilename = folder + '\\magicsuffix.txt';
    ...
    // from https://github.com/olegbl/d2rmm.mods/blob/main/HoradricForging/mod.js
        ...
        const uniqueitemsFilename = folder + '\\uniqueitems.txt';
...
processFolder('global\\excel');

const didUpdateRecipesPanel = updateRecipesPanel;
updateRecipesPanel = false;

processFolder('global\\excel\\base');

if (didUpdateRecipesPanel) {
