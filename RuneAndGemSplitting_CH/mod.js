...
function FixGemUpgradeRecipe(cubemainTsv, gemType, tier) {
...
function processFolder(folder) {
    ...
    if (config.runeUpgrades == 'two') {
        for (let tier = 1; tier <= 32; tier++)
            FixRuneUpgradeRecipe(cubemain, tier, 2);
    }
    else if (config.runeUpgrades == 'noGems') {
        for (let tier = 1; tier <= 32; tier++)
            FixRuneUpgradeRecipe(cubemain, tier, tier >= 21 ? 2 : 3);
    }
    ...
    if (config.twoGemUpgrades) {
        Object.keys(GEM_TYPES).forEach((gemType) => {
            for (let tier = 0; tier <= 3; tier++)
                FixGemUpgradeRecipe(cubemain, gemType, tier);
        })
    }

    if (config.gemSplitting) {
        console.log(typeof GEM_TYPES);
